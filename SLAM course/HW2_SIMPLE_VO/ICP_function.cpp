...
void get_cloud(std::vector<std::string> & depth_name, int picture_ind, PointCloud::Ptr PointCloud)
{
	...
	//std::cout << "K " << Camera_K<<std::endl;
	//std::cout << "K(0,1) " << Camera_K(0,2) << std::endl;
	//std::cout << "K1 " << Camera_K(2) << std::endl;
...
Eigen::Matrix4d ICP_iterate_p2p(PointCloud::Ptr source_corres, PointCloud::Ptr target_corres)
{
	...
	Eigen::Matrix4d ICP_transformation;
	...
	ICP_transformation << ICP_R(0, 0), ICP_R(0, 1), ICP_R(0, 2), ICP_T[0],
		...
		ICP_R(2, 0), ICP_R(2, 1), ICP_R(2, 2), ICP_T[2],
		0, 0, 0, 1;

	//Eigen::Matrix3d ICP_R_inv = ICP_R.inverse();
	//Eigen::Vector3d ICP_T_inv = ICP_T.inverse();
	//ICP_transformation << ICP_R_inv(0, 0), ICP_R_inv(0, 1), ICP_R_inv(0, 2), ICP_T_inv[0],
	//	ICP_R_inv(1, 0), ICP_R_inv(1, 1), ICP_R_inv(1, 2), ICP_T_inv[1],
	//	ICP_R_inv(2, 1), ICP_R_inv(2, 1), ICP_R_inv(2, 2), ICP_T_inv[2],
	//	0, 0, 0, 1;
...
Eigen::Matrix4d ICP_iterate_P2L(PointCloud::Ptr source_cloud, PointCloud::Ptr target_corres, pcl::PointCloud<pcl::Normal>::Ptr Normals)
{
	...
	Eigen::Matrix4d ICP_transformation;
	// column master MEANING!!!!
	ICP_transformation << R(0, 0), R(0, 1), R(0, 2), x_opt(3),
		R(1, 0), R(1, 1), R(1, 2), x_opt(4),
		R(2, 0), R(2, 1), R(2, 2), x_opt(5),
		0, 0, 0, 1;

	//if x_opt returns nan nad ,use p2p algorithm;
	if (R(0, 0) != R(0, 0))
	{
		std::cout << "non value, no solution to Ax=b" << std::endl;
		ICP_transformation = ICP_iterate_p2p(source_cloud, target_corres);
	}
