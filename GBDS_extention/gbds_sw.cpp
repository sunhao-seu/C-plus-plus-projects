...
static int count_cell_size[k_sub_region_max];		//count the number of data in each grid
static int data_ordered_by_hash_PL[k_data_set_size];	//reorder the data's index by hash
static int cell_first_index[k_sub_region_max];		//the first index of each cell when merge them in ordered hash array.
...
void BuildGBDS_sw(struct ThreeDimPoint data_set[k_data_set_size], int data_set_size, unsigned int rand_seed, float split_precise)
{
	...
	DataClassify_hash_sw(data_set, data_set_size, x_split_array_PL, y_split_array_PL, z_split_array_PL, split_array_size_PL);
...
void DataClassify_hash_sw(struct ThreeDimPoint data_set[], int data_set_size, type_point x_split_array[], type_point y_split_array[], type_point z_split_array[], struct SplitArraySize split_array_size)
{
	//static int data_hash_PL[k_data_set_size];			//each data's hash
	//static int count_cell_size[k_sub_region_max];		//count the number of data in each grid
	//static int data_ordered_by_hash_PL[k_data_set_size];	//reorder the data's index by hash;  for debug
	//static int cell_first_index[k_sub_region_max];		//the first index of each cell when merge them in ordered hash array.
	//static struct ThreeDimPoint ordered_data_set_PL[k_data_set_size];	//new dataset ordered by hash. 

	int cell_occupied_number[k_sub_region_max];		//the first index of each grid when merge them in ordered hash array.
...
//	static int data_ordered_by_hash_PL[k_data_set_size];
	...
	for (int i = 0; i < data_set_size; i++)
	{
		...
		data_hash_PL[i] = data_index;
		count_cell_size[data_index] = count_cell_size[data_index] + 1;
	}

	cell_first_index[0] = 0;
	for (int i = 1; i < k_sub_region_max; i++)
	{
	...
	for (int i = 0; i < data_set_size; i++)
	{
		...
		ordered_data_set_PL[insert_index] = data_set[i];
...
#pragma HLS loop_tripcount min=1 max=40
	...
	for (int i = 0; i < valid_near_region_size; i++)
	{
		...
		for (int j = 0; j < count_cell_size[i]; j++)
		{
			int index = data_ordered_by_hash_PL[cell_first_index[i] + j];
			type_point distance = PCLEucDist(query_data_PL, ordered_data_set_PL[cell_first_index[i] + j]);
			//if distance is smaller than the biggest distance in result array,
			//then store the distance and the index of the point to replace the old biggest one.
