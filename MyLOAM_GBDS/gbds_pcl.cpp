...
void SplitSubSpacePrecise(const int data_set_size, const struct MaxMin data_max_min, type_point x_split_array[], type_point y_split_array[], type_point z_split_array[], struct SplitArraySize & split_array_size)
{
	...
	int total_sub_spaces = ((x_split_size+1) * (y_split_size + 1) * (z_split_size + 1));	//原来是这个地方的问题。。写代码还是得思虑周全啊。。

	//ensure the number of sub-spaces won't exceed the k_sub_region_max
	if (total_sub_spaces > k_sub_region_max)
	{
		int split_down_size = ceil(pow((k_sub_region_max / total_sub_spaces),1/3)) +1;	//好像是这个地方的问题。。
		x_split_size = x_split_size / split_down_size -1;
		y_split_size = y_split_size / split_down_size -1;		//这个小buG找了我好久，。。我说怎么会算出来超过范围的数;;
...
void DataClassify(struct ThreeDimPoint data_set[], int data_set_size, type_point x_split_array[], type_point y_split_array[], type_point z_split_array[], struct SplitArraySize split_array_size, int sub_sets[][k_sub_space_array_size], int sub_sets_size[])
{
	...
	for (int i = 0; i < data_set_size; i++)
	{
		...
		if (sub_sets_size[data_index] < 1.0 / 3 * k_sub_space_array_size)		//存储数据在0到1/3*k_sub_space_array_size之间
		{
			sub_sets[data_index][sub_sets_size[data_index]] = i;
		}
		else if (sub_sets_size[data_index] < 2.0 / 3 * k_sub_space_array_size)	//存储数据在1/3*k_sub_space_array_size到2/3*k_sub_space_array_size之间
		{
			//三分之一的概率存起来
		...
		else if (sub_sets_size[data_index] < 4.0 / 5 * k_sub_space_array_size)	//存储数据在2/3*k_sub_space_array_size到4/5*k_sub_space_array_size之间
		{
			//五分之一的概率存起来
			int rand_number = rand() % 10;
			if (rand_number < 2)
		...
		else if(sub_sets_size[data_index] < k_sub_space_array_size)	//存储数据在4/5*k_sub_space_array_size到k_sub_space_array_size之间
		{
			//10分之一的概率存起来
			int rand_number = rand() % 10;
			if (rand_number < 1)
